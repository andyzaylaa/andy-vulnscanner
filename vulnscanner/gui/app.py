...
from vulnscanner.reports.pdf_generator import PDFReportGenerator
...
from vulnscanner.scanner.recon_scanner import ReconFinding, ReconScanner, ReconScanResult
from vulnscanner.scanner.network_scanner import NetworkSniffer
from vulnscanner.scanner.bruteforce_scanner import BruteFinding, BruteForceScanner, BruteResult
from vulnscanner.scanner.waf_cms_scanner import WafCmsFinding, WafCmsScanner, WafCmsResult
from vulnscanner.scanner.integrations import IntegrationFinding, ShodanScanner, IntegrationResult
from vulnscanner.scheduler import ScanScheduler
from vulnscanner.vuln_db import VulnDatabase
...
class AndyVulnScanner(tk.Tk):
    ...
    def __init__(self):
        ...
        self.brute_scanner = BruteForceScanner()
        self.waf_cms_scanner = WafCmsScanner()
        ...
        self.current_recon_result: Optional[ReconScanResult] = None
        self.current_brute_result: Optional[BruteResult] = None
        self.current_waf_cms_result: Optional[WafCmsResult] = None
        self.current_shodan_result: Optional[IntegrationResult] = None
        self.is_scanning = False
        self.scan_thread: Optional[threading.Thread] = None
        self.all_findings: list[dict] = []  # Aggregated findings for PDF report
    ...
    def _build_notebook(self):
        ...
        # Tab 6: WHOIS & DNS Recon
        self.recon_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.recon_frame, text="  WHOIS/DNS  ")
        ...
        # Tab 8: WAF & CMS
        self.waf_cms_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.waf_cms_frame, text="  WAF/CMS  ")
        self._build_waf_cms_tab()

        # Tab 9: Shodan
        self.shodan_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.shodan_frame, text="  Shodan  ")
        self._build_shodan_tab()

        # Tab 10: Vuln DB
        self.vulndb_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.vulndb_frame, text="  Vuln DB  ")
        self._build_vulndb_tab()
    ...
    def _build_recon_tab(self):
        """Build the WHOIS/DNS Recon & Email Harvester tab."""
        # Top config bar
        recon_config = ttk.Frame(self.recon_frame, style="Card.TFrame")
        recon_config.pack(fill=tk.X, padx=5, pady=(5, 3))

        config_inner = ttk.Frame(recon_config, style="Card.TFrame")
        config_inner.pack(fill=tk.X, padx=10, pady=8)

        ttk.Label(config_inner, text="Domain:", style="Card.TLabel",
                  font=("Segoe UI", 10, "bold")).pack(side=tk.LEFT)
        self.recon_target_entry = ttk.Entry(config_inner, width=35,
                                            font=("Consolas", 11))
        self.recon_target_entry.pack(side=tk.LEFT, padx=(8, 10))
        self.recon_target_entry.insert(0, "example.com")
        self.recon_target_entry.bind("<Return>",
                                     lambda e: self._start_recon_scan())

        self.recon_scan_button = ttk.Button(
            config_inner, text="Run Recon",
            style="Scan.TButton", command=self._start_recon_scan,
        )
        self.recon_scan_button.pack(side=tk.RIGHT, padx=(5, 0))

        self.recon_stop_button = ttk.Button(
            config_inner, text="Stop",
            style="Stop.TButton", command=self._stop_recon_scan,
            state="disabled",
        )
        self.recon_stop_button.pack(side=tk.RIGHT, padx=(5, 0))

        # Checkboxes
        checks_frame = ttk.Frame(self.recon_frame, style="Card.TFrame")
        checks_frame.pack(fill=tk.X, padx=5, pady=(0, 3))
        checks_inner = ttk.Frame(checks_frame, style="Card.TFrame")
        checks_inner.pack(fill=tk.X, padx=10, pady=5)

        self.recon_whois_var = tk.BooleanVar(value=True)
        self.recon_dns_var = tk.BooleanVar(value=True)
        self.recon_email_var = tk.BooleanVar(value=True)

        for label, var in [("WHOIS Lookup", self.recon_whois_var),
                           ("DNS Recon", self.recon_dns_var),
                           ("Email Harvester", self.recon_email_var)]:
            cb = tk.Checkbutton(
                checks_inner, text=label, variable=var,
                bg=DarkTheme.BG_SECONDARY, fg=DarkTheme.FG_PRIMARY,
                selectcolor=DarkTheme.BG_INPUT,
        ...
        # Summary
        self.recon_summary = ttk.Label(
            self.recon_frame,
            text="Enter a domain and click Run Recon.",
            font=("Segoe UI", 10), foreground=DarkTheme.FG_SECONDARY,
        )
        self.recon_summary.pack(fill=tk.X, padx=10, pady=(5, 3))

        # Treeview
        columns = ("severity", "scanner", "title", "description")
        self.recon_tree = ttk.Treeview(
            self.recon_frame, columns=columns, show="headings", height=10,
        )
        self.recon_tree.heading("severity", text="Severity")
        self.recon_tree.heading("scanner", text="Scanner")
        self.recon_tree.heading("title", text="Finding")
        self.recon_tree.heading("description", text="Details")
        self.recon_tree.column("severity", width=70, anchor="center")
        self.recon_tree.column("scanner", width=90, anchor="center")
        self.recon_tree.column("title", width=250)
        self.recon_tree.column("description", width=400)

        for sev in ["Critical", "High", "Medium", "Low", "Info"]:
            self.recon_tree.tag_configure(sev, foreground={
                "Critical": "#ff4444", "High": "#ff8800",
                "Medium": "#ffcc00", "Low": "#00aaff", "Info": "#00cc00",
            }[sev])

        recon_scroll = ttk.Scrollbar(self.recon_frame, orient=tk.VERTICAL,
                                     command=self.recon_tree.yview)
        self.recon_tree.configure(yscrollcommand=recon_scroll.set)
        self.recon_tree.pack(fill=tk.BOTH, expand=True, padx=10, side=tk.LEFT)
        recon_scroll.pack(fill=tk.Y, side=tk.RIGHT, padx=(0, 10))

        # Detail panel
        self.recon_detail = scrolledtext.ScrolledText(
            self.recon_frame, height=5, wrap=tk.WORD,
            bg=DarkTheme.BG_SECONDARY, fg=DarkTheme.FG_PRIMARY,
            font=("Consolas", 10), insertbackground=DarkTheme.FG_PRIMARY,
            relief="flat", state="disabled",
        )
        self.recon_detail.pack(fill=tk.X, padx=10, pady=(3, 5), side=tk.BOTTOM)
        self.recon_tree.bind("<<TreeviewSelect>>", self._on_recon_finding_select)

    def _build_brute_tab(self):
        """Build the Brute Force Login tab."""
        brute_config = ttk.Frame(self.brute_frame, style="Card.TFrame")
        brute_config.pack(fill=tk.X, padx=5, pady=(5, 3))

        config_inner = ttk.Frame(brute_config, style="Card.TFrame")
        config_inner.pack(fill=tk.X, padx=10, pady=8)

        ttk.Label(config_inner, text="Target:", style="Card.TLabel",
                  font=("Segoe UI", 10, "bold")).pack(side=tk.LEFT)
        self.brute_target_entry = ttk.Entry(config_inner, width=35,
                                            font=("Consolas", 11))
        self.brute_target_entry.pack(side=tk.LEFT, padx=(8, 10))
        self.brute_target_entry.insert(0, "https://example.com")
        self.brute_target_entry.bind("<Return>",
                                     lambda e: self._start_brute_scan())

        self.brute_scan_button = ttk.Button(
            config_inner, text="Run Brute Force",
            style="Scan.TButton", command=self._start_brute_scan,
        )
        self.brute_scan_button.pack(side=tk.RIGHT, padx=(5, 0))

        self.brute_stop_button = ttk.Button(
            config_inner, text="Stop",
            style="Stop.TButton", command=self._stop_brute_scan,
            state="disabled",
        )
        self.brute_stop_button.pack(side=tk.RIGHT, padx=(5, 0))

        # Checkboxes
        checks_frame = ttk.Frame(self.brute_frame, style="Card.TFrame")
        checks_frame.pack(fill=tk.X, padx=5, pady=(0, 3))
        checks_inner = ttk.Frame(checks_frame, style="Card.TFrame")
        checks_inner.pack(fill=tk.X, padx=10, pady=5)

        self.brute_http_var = tk.BooleanVar(value=True)
        self.brute_ftp_var = tk.BooleanVar(value=True)
        self.brute_ssh_var = tk.BooleanVar(value=True)

        for label, var in [("HTTP Login", self.brute_http_var),
                           ("FTP", self.brute_ftp_var),
                           ("SSH", self.brute_ssh_var)]:
            cb = tk.Checkbutton(
                checks_inner, text=label, variable=var,
                bg=DarkTheme.BG_SECONDARY, fg=DarkTheme.FG_PRIMARY,
                selectcolor=DarkTheme.BG_INPUT,
        ...
        # Summary
        self.brute_summary = ttk.Label(
            self.brute_frame,
            text="Enter target and click Run Brute Force.",
            font=("Segoe UI", 10), foreground=DarkTheme.FG_SECONDARY,
        )
        self.brute_summary.pack(fill=tk.X, padx=10, pady=(5, 3))

        # Treeview
        columns = ("severity", "scanner", "title", "description")
        self.brute_tree = ttk.Treeview(
            self.brute_frame, columns=columns, show="headings", height=10,
        )
        self.brute_tree.heading("severity", text="Severity")
        self.brute_tree.heading("scanner", text="Scanner")
        self.brute_tree.heading("title", text="Finding")
        self.brute_tree.heading("description", text="Details")
        self.brute_tree.column("severity", width=70, anchor="center")
        self.brute_tree.column("scanner", width=90, anchor="center")
        self.brute_tree.column("title", width=250)
        self.brute_tree.column("description", width=400)

        for sev in ["Critical", "High", "Medium", "Low", "Info"]:
            self.brute_tree.tag_configure(sev, foreground={
                "Critical": "#ff4444", "High": "#ff8800",
                "Medium": "#ffcc00", "Low": "#00aaff", "Info": "#00cc00",
            }[sev])

        brute_scroll = ttk.Scrollbar(self.brute_frame, orient=tk.VERTICAL,
                                     command=self.brute_tree.yview)
        self.brute_tree.configure(yscrollcommand=brute_scroll.set)
        self.brute_tree.pack(fill=tk.BOTH, expand=True, padx=10, side=tk.LEFT)
        brute_scroll.pack(fill=tk.Y, side=tk.RIGHT, padx=(0, 10))

        # Detail panel
        self.brute_detail = scrolledtext.ScrolledText(
            self.brute_frame, height=5, wrap=tk.WORD,
            bg=DarkTheme.BG_SECONDARY, fg=DarkTheme.FG_PRIMARY,
            font=("Consolas", 10), insertbackground=DarkTheme.FG_PRIMARY,
            relief="flat", state="disabled",
        )
        self.brute_detail.pack(fill=tk.X, padx=10, pady=(3, 5), side=tk.BOTTOM)
        self.brute_tree.bind("<<TreeviewSelect>>", self._on_brute_finding_select)

    def _build_waf_cms_tab(self):
        """Build the WAF Detection & CMS Scanner tab."""
        waf_config = ttk.Frame(self.waf_cms_frame, style="Card.TFrame")
        waf_config.pack(fill=tk.X, padx=5, pady=(5, 3))

        config_inner = ttk.Frame(waf_config, style="Card.TFrame")
        config_inner.pack(fill=tk.X, padx=10, pady=8)

        ttk.Label(config_inner, text="URL:", style="Card.TLabel",
                  font=("Segoe UI", 10, "bold")).pack(side=tk.LEFT)
        self.waf_target_entry = ttk.Entry(config_inner, width=35,
                                          font=("Consolas", 11))
        self.waf_target_entry.pack(side=tk.LEFT, padx=(8, 10))
        self.waf_target_entry.insert(0, "https://example.com")
        self.waf_target_entry.bind("<Return>",
                                   lambda e: self._start_waf_cms_scan())

        self.waf_scan_button = ttk.Button(
            config_inner, text="Run WAF/CMS Scan",
            style="Scan.TButton", command=self._start_waf_cms_scan,
        )
        self.waf_scan_button.pack(side=tk.RIGHT, padx=(5, 0))

        self.waf_stop_button = ttk.Button(
            config_inner, text="Stop",
            style="Stop.TButton", command=self._stop_waf_cms_scan,
            state="disabled",
        )
        self.waf_stop_button.pack(side=tk.RIGHT, padx=(5, 0))

        # Checkboxes
        checks_frame = ttk.Frame(self.waf_cms_frame, style="Card.TFrame")
        checks_frame.pack(fill=tk.X, padx=5, pady=(0, 3))
        checks_inner = ttk.Frame(checks_frame, style="Card.TFrame")
        checks_inner.pack(fill=tk.X, padx=10, pady=5)

        self.waf_detect_var = tk.BooleanVar(value=True)
        self.cms_scan_var = tk.BooleanVar(value=True)

        for label, var in [("WAF Detection", self.waf_detect_var),
                           ("CMS Vulnerability Scan", self.cms_scan_var)]:
            cb = tk.Checkbutton(
                checks_inner, text=label, variable=var,
                bg=DarkTheme.BG_SECONDARY, fg=DarkTheme.FG_PRIMARY,
                selectcolor=DarkTheme.BG_INPUT,
        ...
        # Summary
        self.waf_summary = ttk.Label(
            self.waf_cms_frame,
            text="Enter URL and click Run WAF/CMS Scan.",
            font=("Segoe UI", 10), foreground=DarkTheme.FG_SECONDARY,
        )
        self.waf_summary.pack(fill=tk.X, padx=10, pady=(5, 3))

        # Treeview
        columns = ("severity", "scanner", "title", "description")
        self.waf_tree = ttk.Treeview(
            self.waf_cms_frame, columns=columns, show="headings", height=10,
        )
        self.waf_tree.heading("severity", text="Severity")
        self.waf_tree.heading("scanner", text="Scanner")
        self.waf_tree.heading("title", text="Finding")
        self.waf_tree.heading("description", text="Details")
        self.waf_tree.column("severity", width=70, anchor="center")
        self.waf_tree.column("scanner", width=90, anchor="center")
        self.waf_tree.column("title", width=250)
        self.waf_tree.column("description", width=400)

        for sev in ["Critical", "High", "Medium", "Low", "Info"]:
            self.waf_tree.tag_configure(sev, foreground={
                "Critical": "#ff4444", "High": "#ff8800",
                "Medium": "#ffcc00", "Low": "#00aaff", "Info": "#00cc00",
            }[sev])

        waf_scroll = ttk.Scrollbar(self.waf_cms_frame, orient=tk.VERTICAL,
                                    command=self.waf_tree.yview)
        self.waf_tree.configure(yscrollcommand=waf_scroll.set)
        self.waf_tree.pack(fill=tk.BOTH, expand=True, padx=10, side=tk.LEFT)
        waf_scroll.pack(fill=tk.Y, side=tk.RIGHT, padx=(0, 10))

        # Detail panel
        self.waf_detail = scrolledtext.ScrolledText(
            self.waf_cms_frame, height=5, wrap=tk.WORD,
            bg=DarkTheme.BG_SECONDARY, fg=DarkTheme.FG_PRIMARY,
            font=("Consolas", 10), insertbackground=DarkTheme.FG_PRIMARY,
            relief="flat", state="disabled",
        )
        self.waf_detail.pack(fill=tk.X, padx=10, pady=(3, 5), side=tk.BOTTOM)
        self.waf_tree.bind("<<TreeviewSelect>>", self._on_waf_finding_select)

    def _build_shodan_tab(self):
        """Build the Shodan Integration tab."""
        shodan_config = ttk.Frame(self.shodan_frame, style="Card.TFrame")
        shodan_config.pack(fill=tk.X, padx=5, pady=(5, 3))

        config_inner = ttk.Frame(shodan_config, style="Card.TFrame")
        config_inner.pack(fill=tk.X, padx=10, pady=8)

        ttk.Label(config_inner, text="Target:", style="Card.TLabel",
                  font=("Segoe UI", 10, "bold")).pack(side=tk.LEFT)
        self.shodan_target_entry = ttk.Entry(config_inner, width=25,
                                             font=("Consolas", 11))
        self.shodan_target_entry.pack(side=tk.LEFT, padx=(8, 10))
        self.shodan_target_entry.insert(0, "example.com")

        ttk.Label(config_inner, text="API Key:", style="Card.TLabel",
                  font=("Segoe UI", 10, "bold")).pack(side=tk.LEFT)
        self.shodan_api_entry = ttk.Entry(config_inner, width=25,
                                          font=("Consolas", 11), show="*")
        self.shodan_api_entry.pack(side=tk.LEFT, padx=(8, 10))

        self.shodan_scan_button = ttk.Button(
            config_inner, text="Query Shodan",
            style="Scan.TButton", command=self._start_shodan_scan,
        )
        self.shodan_scan_button.pack(side=tk.RIGHT, padx=(5, 0))

        self.shodan_stop_button = ttk.Button(
            config_inner, text="Stop",
            style="Stop.TButton", command=self._stop_shodan_scan,
            state="disabled",
        )
        self.shodan_stop_button.pack(side=tk.RIGHT, padx=(5, 0))

        # Summary
        self.shodan_summary = ttk.Label(
            self.shodan_frame,
            text="Enter target and Shodan API key, then click Query Shodan.",
            font=("Segoe UI", 10), foreground=DarkTheme.FG_SECONDARY,
        )
        self.shodan_summary.pack(fill=tk.X, padx=10, pady=(5, 3))

        # Treeview
        columns = ("severity", "scanner", "title", "description")
        self.shodan_tree = ttk.Treeview(
            self.shodan_frame, columns=columns, show="headings", height=10,
        )
        self.shodan_tree.heading("severity", text="Severity")
        self.shodan_tree.heading("scanner", text="Scanner")
        self.shodan_tree.heading("title", text="Finding")
        self.shodan_tree.heading("description", text="Details")
        self.shodan_tree.column("severity", width=70, anchor="center")
        self.shodan_tree.column("scanner", width=90, anchor="center")
        self.shodan_tree.column("title", width=250)
        self.shodan_tree.column("description", width=400)

        for sev in ["Critical", "High", "Medium", "Low", "Info"]:
            self.shodan_tree.tag_configure(sev, foreground={
                "Critical": "#ff4444", "High": "#ff8800",
                "Medium": "#ffcc00", "Low": "#00aaff", "Info": "#00cc00",
            }[sev])

        shodan_scroll = ttk.Scrollbar(self.shodan_frame, orient=tk.VERTICAL,
                                      command=self.shodan_tree.yview)
        self.shodan_tree.configure(yscrollcommand=shodan_scroll.set)
        self.shodan_tree.pack(fill=tk.BOTH, expand=True, padx=10, side=tk.LEFT)
        shodan_scroll.pack(fill=tk.Y, side=tk.RIGHT, padx=(0, 10))

        # Detail panel
        self.shodan_detail = scrolledtext.ScrolledText(
            self.shodan_frame, height=5, wrap=tk.WORD,
            bg=DarkTheme.BG_SECONDARY, fg=DarkTheme.FG_PRIMARY,
            font=("Consolas", 10), insertbackground=DarkTheme.FG_PRIMARY,
            relief="flat", state="disabled",
        )
        self.shodan_detail.pack(fill=tk.X, padx=10, pady=(3, 5), side=tk.BOTTOM)
        self.shodan_tree.bind("<<TreeviewSelect>>", self._on_shodan_finding_select)

    def _build_vulndb_tab(self):
        """Build the Vulnerability Database tab."""
        db_config = ttk.Frame(self.vulndb_frame, style="Card.TFrame")
        db_config.pack(fill=tk.X, padx=5, pady=(5, 3))

        config_inner = ttk.Frame(db_config, style="Card.TFrame")
        config_inner.pack(fill=tk.X, padx=10, pady=8)

        ttk.Label(config_inner, text="Search CVE:", style="Card.TLabel",
                  font=("Segoe UI", 10, "bold")).pack(side=tk.LEFT)
        self.vulndb_search_entry = ttk.Entry(config_inner, width=25,
                                              font=("Consolas", 11))
        self.vulndb_search_entry.pack(side=tk.LEFT, padx=(8, 10))
        self.vulndb_search_entry.insert(0, "apache")
        self.vulndb_search_entry.bind("<Return>",
                                      lambda e: self._search_vulndb())

        ttk.Button(
            config_inner, text="Search",
            style="Scan.TButton", command=self._search_vulndb,
        ).pack(side=tk.LEFT, padx=(0, 10))

        self.vulndb_update_button = ttk.Button(
            config_inner, text="Update DB from NVD",
            style="Scan.TButton", command=self._update_vulndb,
        )
        self.vulndb_update_button.pack(side=tk.RIGHT, padx=(5, 0))

        ttk.Button(
            config_inner, text="Seed Built-in CVEs",
            style="Export.TButton", command=self._seed_vulndb,
        ).pack(side=tk.RIGHT, padx=(5, 0))

        # DB Info
        self.vulndb_info = ttk.Label(
            self.vulndb_frame,
            text="Loading database info...",
            font=("Segoe UI", 10), foreground=DarkTheme.FG_SECONDARY,
        )
        self.vulndb_info.pack(fill=tk.X, padx=10, pady=(5, 3))
        self._refresh_vulndb_info()

        # Treeview
        columns = ("cve_id", "severity", "cvss", "published", "description")
        self.vulndb_tree = ttk.Treeview(
            self.vulndb_frame, columns=columns, show="headings", height=12,
        )
        self.vulndb_tree.heading("cve_id", text="CVE ID")
        self.vulndb_tree.heading("severity", text="Severity")
        self.vulndb_tree.heading("cvss", text="CVSS")
        self.vulndb_tree.heading("published", text="Published")
        self.vulndb_tree.heading("description", text="Description")
        self.vulndb_tree.column("cve_id", width=140, anchor="center")
        self.vulndb_tree.column("severity", width=80, anchor="center")
        self.vulndb_tree.column("cvss", width=60, anchor="center")
        self.vulndb_tree.column("published", width=100, anchor="center")
        self.vulndb_tree.column("description", width=450)

        for sev in ["Critical", "High", "Medium", "Low", "Info", "Unknown"]:
            color = {
                "Critical": "#ff4444", "High": "#ff8800",
                "Medium": "#ffcc00", "Low": "#00aaff",
                "Info": "#00cc00", "Unknown": "#888888",
            }.get(sev, "#888888")
            self.vulndb_tree.tag_configure(sev, foreground=color)

        db_scroll = ttk.Scrollbar(self.vulndb_frame, orient=tk.VERTICAL,
                                   command=self.vulndb_tree.yview)
        self.vulndb_tree.configure(yscrollcommand=db_scroll.set)
        self.vulndb_tree.pack(fill=tk.BOTH, expand=True, padx=10, side=tk.LEFT)
        db_scroll.pack(fill=tk.Y, side=tk.RIGHT, padx=(0, 10))

        # Detail panel
        self.vulndb_detail = scrolledtext.ScrolledText(
            self.vulndb_frame, height=5, wrap=tk.WORD,
            bg=DarkTheme.BG_SECONDARY, fg=DarkTheme.FG_PRIMARY,
            font=("Consolas", 10), insertbackground=DarkTheme.FG_PRIMARY,
            relief="flat", state="disabled",
        )
        self.vulndb_detail.pack(fill=tk.X, padx=10, pady=(3, 5), side=tk.BOTTOM)
        self.vulndb_tree.bind("<<TreeviewSelect>>", self._on_vulndb_select)
    ...
    def _build_export_tab(self):
        ...
        # PDF Report section
        ttk.Separator(export_inner, orient="horizontal").pack(
            fill=tk.X, pady=20, padx=50)

        ttk.Label(export_inner, text="Professional Assessment Report",
                  font=("Segoe UI", 14, "bold"),
                  foreground=DarkTheme.FG_ACCENT).pack(pady=(0, 10))

        ttk.Label(export_inner,
                  text="Generate a comprehensive vulnerability assessment report"
                  " from all scan results.",
                  foreground=DarkTheme.FG_SECONDARY).pack(pady=(0, 15))

        ttk.Button(export_inner,
                   text="Generate Assessment Report (PDF-ready)",
                   style="Scan.TButton",
    ...
    def _disable_all_scan_buttons(self):
        """Disable all scan buttons to prevent concurrent scans."""
        for btn in [self.scan_button, self.web_scan_button,
                    self.adv_scan_button, self.extra_scan_button,
                    self.recon_scan_button, self.brute_scan_button,
                    self.waf_scan_button, self.shodan_scan_button]:
            btn.configure(state="disabled")

    def _enable_all_scan_buttons(self):
    ...
    # ── Recon Scanning (WHOIS, DNS, Email) ────────────────────────────

    def _start_recon_scan(self):
        """Start a WHOIS/DNS recon scan."""
        ...
        self.is_scanning = True
        self._disable_all_scan_buttons()
        self.recon_stop_button.configure(state="normal")
        self.progress_bar["value"] = 0

        for item in self.recon_tree.get_children():
            self.recon_tree.delete(item)
        self.recon_summary.configure(text="Scanning...",
                                     foreground=DarkTheme.FG_SECONDARY)
        self.recon_detail.configure(state="normal")
        self.recon_detail.delete("1.0", tk.END)
        self.recon_detail.configure(state="disabled")
        self.current_recon_result = None
        self.console_text.delete("1.0", tk.END)

        self._log(f"{'=' * 55}", "header")
        self._log("  ANDY VULNSCANNER - WHOIS/DNS Recon", "header")
        self._log(f"{'=' * 55}", "header")
        self._log(f"Target: {target}", "info")
        self._log("")
        self.status_label.configure(text=f"Recon scanning {target}...")
        ...
        self.recon_scanner = ReconScanner(
            on_finding=self._on_recon_finding,
            on_progress=self._on_scan_progress,
            on_log=self._log,
        )
        ...
        self.scan_thread = threading.Thread(
            ...
            args=(target, self.recon_whois_var.get(),
                  self.recon_dns_var.get(), self.recon_email_var.get()),
            daemon=True,
        )
        self.scan_thread.start()

    def _run_recon_scan(self, target: str, whois: bool, dns: bool, email: bool):
        """Run recon scan in background."""
        start_time = time.time()
        try:
            result = self.recon_scanner.scan(
                target, whois=whois, dns_recon=dns, email_harvest=email,
            )
            result.scan_time = time.time() - start_time
            self.current_recon_result = result
            self._log("")
            self._log(f"Recon scan completed in {result.scan_time:.2f}s", "success")
            self._log(f"Findings: {len(result.findings)}",
                      "warning" if result.findings else "success")
            self.after(0, lambda: self._display_recon_findings(result))
        except Exception as exc:
            self._log(f"Error: {exc}", "error")
            msg = str(exc)
            self.after(0, lambda: messagebox.showerror("Recon Error", msg))
        finally:
            self.after(0, self._recon_scan_finished)

    def _recon_scan_finished(self):
        self.is_scanning = False
        self._enable_all_scan_buttons()
        self.recon_stop_button.configure(state="disabled")
        self.progress_bar["value"] = 100
        self.status_label.configure(text="Recon scan complete")

    def _stop_recon_scan(self):
        if self.is_scanning:
            self.recon_scanner.stop()
            self._log("\nRecon scan stopped by user.", "warning")
            self.status_label.configure(text="Recon scan stopped")

    def _on_recon_finding(self, finding: ReconFinding):
        self.after(0, lambda f=finding: self.recon_tree.insert(
            "", tk.END,
            values=(f.severity, f.scanner.upper(), f.title,
                    f.description[:100]),
            tags=(f.severity,),
    ...
    def _on_recon_finding_select(self, event=None):
        selection = self.recon_tree.selection()
        if not selection or not self.current_recon_result:
            return
        values = self.recon_tree.item(selection[0], "values")
        if not values:
            return
        for finding in self.current_recon_result.findings:
            if finding.title == values[2]:
                self.recon_detail.configure(state="normal")
                self.recon_detail.delete("1.0", tk.END)
                detail = (
                ...
                self.recon_detail.insert("1.0", detail)
                self.recon_detail.configure(state="disabled")
                break

    def _display_recon_findings(self, result: ReconScanResult):
        for item in self.recon_tree.get_children():
            self.recon_tree.delete(item)
        for finding in result.findings:
    ...
    def _start_brute_scan(self):
        """Start a brute force scan."""
        ...
        self.is_scanning = True
        self._disable_all_scan_buttons()
        self.brute_stop_button.configure(state="normal")
        self.progress_bar["value"] = 0

        for item in self.brute_tree.get_children():
            self.brute_tree.delete(item)
        self.brute_summary.configure(text="Scanning...",
                                     foreground=DarkTheme.FG_SECONDARY)
        self.brute_detail.configure(state="normal")
        self.brute_detail.delete("1.0", tk.END)
        self.brute_detail.configure(state="disabled")
        self.current_brute_result = None
        self.console_text.delete("1.0", tk.END)

        self._log(f"{'=' * 55}", "header")
        self._log("  ANDY VULNSCANNER - Brute Force Scanner", "header")
        self._log(f"{'=' * 55}", "header")
        self._log(f"Target: {target}", "info")
        self._log("")
        self.status_label.configure(text=f"Brute forcing {target}...")

        self.brute_scanner = BruteForceScanner(
            on_finding=self._on_brute_finding,
            on_progress=self._on_scan_progress,
            on_log=self._log,
        )

        self.scan_thread = threading.Thread(
            target=self._run_brute_scan,
            args=(target, self.brute_http_var.get(),
                  self.brute_ftp_var.get(), self.brute_ssh_var.get()),
            daemon=True,
        )
        self.scan_thread.start()

    def _run_brute_scan(self, target: str, http: bool, ftp: bool, ssh: bool):
        start_time = time.time()
        try:
            result = self.brute_scanner.scan(
                target, http_brute=http, ftp_brute=ftp, ssh_brute=ssh,
            )
            result.scan_time = time.time() - start_time
            self.current_brute_result = result
            self._log("")
            self._log(f"Brute force scan completed in {result.scan_time:.2f}s", "success")
            self._log(f"Findings: {len(result.findings)}",
                      "warning" if result.findings else "success")
            self.after(0, lambda: self._display_brute_findings(result))
        except Exception as exc:
            self._log(f"Error: {exc}", "error")
            msg = str(exc)
            self.after(0, lambda: messagebox.showerror("Brute Force Error", msg))
        finally:
            self.after(0, self._brute_scan_finished)

    def _brute_scan_finished(self):
        self.is_scanning = False
        self._enable_all_scan_buttons()
        self.brute_stop_button.configure(state="disabled")
        self.progress_bar["value"] = 100
        self.status_label.configure(text="Brute force scan complete")

    def _stop_brute_scan(self):
        if self.is_scanning:
            self.brute_scanner.stop()
            self._log("\nBrute force scan stopped by user.", "warning")
            self.status_label.configure(text="Brute force scan stopped")

    def _on_brute_finding(self, finding: BruteFinding):
        self.after(0, lambda f=finding: self.brute_tree.insert(
            "", tk.END,
            values=(f.severity, f.scanner.upper(), f.title,
                    f.description[:100]),
            tags=(f.severity,),
    ...
    def _on_brute_finding_select(self, event=None):
        selection = self.brute_tree.selection()
        if not selection or not self.current_brute_result:
            return
        values = self.brute_tree.item(selection[0], "values")
        if not values:
            return
        for finding in self.current_brute_result.findings:
            if finding.title == values[2]:
                self.brute_detail.configure(state="normal")
                self.brute_detail.delete("1.0", tk.END)
                detail = (
                ...
                self.brute_detail.insert("1.0", detail)
                self.brute_detail.configure(state="disabled")
                break

    def _display_brute_findings(self, result: BruteResult):
        for item in self.brute_tree.get_children():
            self.brute_tree.delete(item)
        for finding in result.findings:
            self.brute_tree.insert(
                "", tk.END,
                values=(finding.severity, finding.scanner.upper(),
                        finding.title, finding.description[:100]),
                tags=(finding.severity,),
    ...
    def _start_waf_cms_scan(self):
        """Start a WAF/CMS scan."""
        target = self.waf_target_entry.get().strip()
        if not target:
        ...
        self.is_scanning = True
        self._disable_all_scan_buttons()
        self.waf_stop_button.configure(state="normal")
        self.progress_bar["value"] = 0

        for item in self.waf_tree.get_children():
            self.waf_tree.delete(item)
        self.waf_summary.configure(text="Scanning...",
                                   foreground=DarkTheme.FG_SECONDARY)
        self.waf_detail.configure(state="normal")
        self.waf_detail.delete("1.0", tk.END)
        self.waf_detail.configure(state="disabled")
        self.current_waf_cms_result = None
        self.console_text.delete("1.0", tk.END)

        self._log(f"{'=' * 55}", "header")
        self._log("  ANDY VULNSCANNER - WAF/CMS Scanner", "header")
        self._log(f"{'=' * 55}", "header")
        self._log(f"Target: {target}", "info")
        self._log("")
        self.status_label.configure(text=f"WAF/CMS scanning {target}...")

        self.waf_cms_scanner = WafCmsScanner(
            on_finding=self._on_waf_finding,
            on_progress=self._on_scan_progress,
            on_log=self._log,
        )

        self.scan_thread = threading.Thread(
            target=self._run_waf_cms_scan,
            args=(target, self.waf_detect_var.get(), self.cms_scan_var.get()),
            daemon=True,
        )
        self.scan_thread.start()

    def _run_waf_cms_scan(self, target: str, waf: bool, cms: bool):
        start_time = time.time()
        try:
            result = self.waf_cms_scanner.scan(
                target, waf_detect=waf, cms_scan=cms,
            )
            result.scan_time = time.time() - start_time
            self.current_waf_cms_result = result
            self._log("")
            self._log(f"WAF/CMS scan completed in {result.scan_time:.2f}s", "success")
            self._log(f"Findings: {len(result.findings)}",
                      "warning" if result.findings else "success")
            if result.waf_detected:
                self._log(f"WAF: {result.waf_detected}", "info")
            if result.cms_detected:
                self._log(f"CMS: {result.cms_detected} {result.cms_version}", "info")
            self.after(0, lambda: self._display_waf_findings(result))
        except Exception as exc:
            self._log(f"Error: {exc}", "error")
            msg = str(exc)
            self.after(0, lambda: messagebox.showerror("WAF/CMS Error", msg))
        finally:
            self.after(0, self._waf_scan_finished)

    def _waf_scan_finished(self):
        self.is_scanning = False
        self._enable_all_scan_buttons()
        self.waf_stop_button.configure(state="disabled")
        self.progress_bar["value"] = 100
        self.status_label.configure(text="WAF/CMS scan complete")

    def _stop_waf_cms_scan(self):
        if self.is_scanning:
            self.waf_cms_scanner.stop()
            self._log("\nWAF/CMS scan stopped by user.", "warning")
            self.status_label.configure(text="WAF/CMS scan stopped")

    def _on_waf_finding(self, finding: WafCmsFinding):
        self.after(0, lambda f=finding: self.waf_tree.insert(
            "", tk.END,
            values=(f.severity, f.scanner.upper(), f.title,
                    f.description[:100]),
            tags=(f.severity,),
    ...
    def _on_waf_finding_select(self, event=None):
        selection = self.waf_tree.selection()
        if not selection or not self.current_waf_cms_result:
            return
        values = self.waf_tree.item(selection[0], "values")
        if not values:
            return
        for finding in self.current_waf_cms_result.findings:
            if finding.title == values[2]:
                self.waf_detail.configure(state="normal")
                self.waf_detail.delete("1.0", tk.END)
                detail = (
                ...
                self.waf_detail.insert("1.0", detail)
                self.waf_detail.configure(state="disabled")
                break

    def _display_waf_findings(self, result: WafCmsResult):
        for item in self.waf_tree.get_children():
            self.waf_tree.delete(item)
        for finding in result.findings:
        ...
        info_parts = [f"Target: {result.target}",
                      f"Findings: {len(result.findings)}"]
        if result.waf_detected:
            info_parts.append(f"WAF: {result.waf_detected}")
        if result.cms_detected:
            info_parts.append(f"CMS: {result.cms_detected}")
        if counts_str:
            info_parts.append(f"({counts_str})")
        info_parts.append(f"Time: {result.scan_time:.2f}s")
        self.waf_summary.configure(
            text="  |  ".join(info_parts),
            foreground=DarkTheme.FG_WARNING if result.findings else DarkTheme.FG_SUCCESS,
        )
    ...
    def _start_shodan_scan(self):
        """Start a Shodan query."""
        target = self.shodan_target_entry.get().strip()
        if not target:
            messagebox.showwarning("Warning", "Please enter a target.")
            return
        if self.is_scanning:
        ...
        self.is_scanning = True
        self._disable_all_scan_buttons()
        self.shodan_stop_button.configure(state="normal")
        self.progress_bar["value"] = 0

        for item in self.shodan_tree.get_children():
            self.shodan_tree.delete(item)
        self.shodan_summary.configure(text="Querying...",
                                      foreground=DarkTheme.FG_SECONDARY)
        self.shodan_detail.configure(state="normal")
        self.shodan_detail.delete("1.0", tk.END)
        self.shodan_detail.configure(state="disabled")
        self.current_shodan_result = None
        self.console_text.delete("1.0", tk.END)

        api_key = self.shodan_api_entry.get().strip()

        self._log(f"{'=' * 55}", "header")
        self._log("  ANDY VULNSCANNER - Shodan Integration", "header")
        self._log(f"{'=' * 55}", "header")
        self._log(f"Target: {target}", "info")
        self._log("")
        self.status_label.configure(text=f"Querying Shodan for {target}...")

        self.shodan_scanner = ShodanScanner(
            api_key=api_key,
            on_finding=self._on_shodan_finding,
            on_progress=self._on_scan_progress,
            on_log=self._log,
        )

        self.scan_thread = threading.Thread(
            target=self._run_shodan_scan,
            args=(target,),
            daemon=True,
        )
        self.scan_thread.start()

    def _run_shodan_scan(self, target: str):
        start_time = time.time()
        try:
            result = self.shodan_scanner.scan(target)
            result.scan_time = time.time() - start_time
            self.current_shodan_result = result
            self._log("")
            self._log(f"Shodan query completed in {result.scan_time:.2f}s", "success")
            self._log(f"Findings: {len(result.findings)}",
                      "warning" if result.findings else "success")
            self.after(0, lambda: self._display_shodan_findings(result))
        except Exception as exc:
            self._log(f"Error: {exc}", "error")
            msg = str(exc)
            self.after(0, lambda: messagebox.showerror("Shodan Error", msg))
        finally:
            self.after(0, self._shodan_scan_finished)

    def _shodan_scan_finished(self):
        self.is_scanning = False
        self._enable_all_scan_buttons()
        self.shodan_stop_button.configure(state="disabled")
        self.progress_bar["value"] = 100
        self.status_label.configure(text="Shodan query complete")

    def _stop_shodan_scan(self):
        if self.is_scanning:
            self.shodan_scanner.stop()
            self._log("\nShodan query stopped by user.", "warning")
            self.status_label.configure(text="Shodan query stopped")

    def _on_shodan_finding(self, finding: IntegrationFinding):
        self.after(0, lambda f=finding: self.shodan_tree.insert(
            "", tk.END,
            values=(f.severity, f.scanner.upper(), f.title,
                    f.description[:100]),
            tags=(f.severity,),
    ...
    def _on_shodan_finding_select(self, event=None):
        selection = self.shodan_tree.selection()
        if not selection or not self.current_shodan_result:
            return
        values = self.shodan_tree.item(selection[0], "values")
        if not values:
            return
        for finding in self.current_shodan_result.findings:
            if finding.title == values[2]:
                self.shodan_detail.configure(state="normal")
                self.shodan_detail.delete("1.0", tk.END)
                detail = (
                ...
                self.shodan_detail.insert("1.0", detail)
                self.shodan_detail.configure(state="disabled")
                break

    def _display_shodan_findings(self, result: IntegrationResult):
        for item in self.shodan_tree.get_children():
            self.shodan_tree.delete(item)
        for finding in result.findings:
            self.shodan_tree.insert(
                "", tk.END,
                values=(finding.severity, finding.scanner.upper(),
                        finding.title, finding.description[:100]),
                tags=(finding.severity,),
        ...
        if result.open_ports:
            info_parts.append(f"Open Ports: {len(result.open_ports)}")
        if counts_str:
            info_parts.append(f"({counts_str})")
        info_parts.append(f"Time: {result.scan_time:.2f}s")
        self.shodan_summary.configure(
            text="  |  ".join(info_parts),
            foreground=DarkTheme.FG_WARNING if result.findings else DarkTheme.FG_SUCCESS,
        )
    ...
    def _refresh_vulndb_info(self):
        """Refresh the vuln DB info label."""
        info = self.vuln_db.get_db_info()
        self.vulndb_info.configure(
            text=(
                f"CVEs in DB: {info['total_cves']}  |  "
                f"Last Update: {info['last_update']}  |  "
                f"Size: {info['db_size_kb']} KB"
            ),
            foreground=DarkTheme.FG_ACCENT,
        )

    def _search_vulndb(self):
        """Search the vulnerability database."""
        keyword = self.vulndb_search_entry.get().strip()
        if not keyword:
            return

        self._log(f"Searching vuln DB for: {keyword}", "info")
        results = self.vuln_db.search_cve(keyword)

        for item in self.vulndb_tree.get_children():
            self.vulndb_tree.delete(item)

        for cve in results:
            self.vulndb_tree.insert(
                "", tk.END,
                values=(cve.cve_id, cve.severity, cve.cvss_score,
                        cve.published, cve.description[:100]),
                tags=(cve.severity,),
            )

        self._log(f"Found {len(results)} CVEs.", "success")
        self._refresh_vulndb_info()

    def _seed_vulndb(self):
        """Seed built-in CVE definitions."""
        self.vuln_db.on_log = self._log
        count = self.vuln_db.seed_builtin_vulns()
        self._refresh_vulndb_info()
        messagebox.showinfo("Vuln DB", f"Seeded {count} built-in CVE definitions.")

    def _update_vulndb(self):
        """Update vuln DB from NVD."""
        if self.is_scanning:
            return
        ...
        self._log(f"{'=' * 55}", "header")
        self._log("  ANDY VULNSCANNER - Vulnerability Database Update", "header")
        self._log(f"{'=' * 55}", "header")
        self._log("")
        self.status_label.configure(text="Updating vulnerability database...")

        self.vuln_db.on_log = self._log
        self.vuln_db.on_progress = self._on_scan_progress

        self.scan_thread = threading.Thread(
            target=self._run_vulndb_update,
            daemon=True,
        )
        self.scan_thread.start()

    def _run_vulndb_update(self):
        try:
            self.vuln_db.update()
            self.after(0, self._refresh_vulndb_info)
        except Exception as exc:
            self._log(f"Error: {exc}", "error")
        finally:
            self.after(0, self._vulndb_update_finished)

    def _vulndb_update_finished(self):
        self.is_scanning = False
        self._enable_all_scan_buttons()
        self.progress_bar["value"] = 100
        self.status_label.configure(text="Database update complete")

    def _on_vulndb_select(self, event=None):
        """Show CVE details when selected."""
        selection = self.vulndb_tree.selection()
        if not selection:
            return
        values = self.vulndb_tree.item(selection[0], "values")
        if not values:
            return
        cve_id = values[0]
        cve = self.vuln_db.lookup_cve(cve_id)
        if cve:
            self.vulndb_detail.configure(state="normal")
            self.vulndb_detail.delete("1.0", tk.END)
            detail = (
                f"CVE ID: {cve.cve_id}\n"
            ...
            self.vulndb_detail.insert("1.0", detail)
            self.vulndb_detail.configure(state="disabled")

    # ── Export ────────────────────────────────────────────────────────

    def _collect_all_findings(self) -> list[dict]:
        """Collect findings from all scan results for PDF report."""
        findings: list[dict] = []

        # From recon scanner
        if self.current_recon_result:
            for f in self.current_recon_result.findings:
                findings.append({
        ...
        # From brute force scanner
        if self.current_brute_result:
            for f in self.current_brute_result.findings:
                findings.append({
                    "scanner": f.scanner, "severity": f.severity,
                    "title": f.title, "description": f.description,
                    "evidence": f.evidence, "recommendation": f.recommendation,
        ...
        # From WAF/CMS scanner
        if self.current_waf_cms_result:
            for f in self.current_waf_cms_result.findings:
                findings.append({
                    "scanner": f.scanner, "severity": f.severity,
                    "title": f.title, "description": f.description,
                    "evidence": f.evidence, "recommendation": f.recommendation,
        ...
        # From Shodan
        if self.current_shodan_result:
            for f in self.current_shodan_result.findings:
                findings.append({
                    "scanner": f.scanner, "severity": f.severity,
                    "title": f.title, "description": f.description,
                    "evidence": f.evidence, "recommendation": f.recommendation,
        ...
        # From web scanner
        if self.current_web_result:
            for f in self.current_web_result.findings:
                findings.append({
                    "scanner": f.category, "severity": f.severity,
                    "title": f.title, "description": f.description,
                    "evidence": "", "recommendation": f.recommendation,
                })

        # From advanced scanner
        if self.current_adv_result:
            for f in self.current_adv_result.findings:
                findings.append({
                    "scanner": f.scanner, "severity": f.severity,
                    "title": f.title, "description": f.description,
                    "evidence": f.evidence, "recommendation": f.recommendation,
        ...
        # From extra scanner
        if self.current_extra_result:
            for f in self.current_extra_result.findings:
                findings.append({
                    "scanner": f.scanner, "severity": f.severity,
                    "title": f.title, "description": f.description,
                    "evidence": f.evidence, "recommendation": f.recommendation,
    ...
    def _export_pdf_report(self):
        """Generate a professional PDF-style assessment report."""
        findings = self._collect_all_findings()
        if not findings:
            messagebox.showinfo("Info",
                                "No findings to export. Run some scans first.")
            return

        target = self.target_entry.get().strip() or "unknown"
        filepath = filedialog.asksaveasfilename(
            defaultextension=".html",
            filetypes=[("HTML Report", "*.html")],
            title="Save Professional Assessment Report",
            initialfile=f"andy_assessment_{target}.html",
        )
        if not filepath:
            return
        ...
        if self.current_result:
            scan_results["Port Scan Target"] = self.current_result.target
            scan_results["Open Ports"] = len(self.current_result.ports)
        if self.current_web_result:
            scan_results["Web Scan URL"] = self.current_web_result.url
        if self.current_waf_cms_result:
            if self.current_waf_cms_result.waf_detected:
                scan_results["WAF Detected"] = self.current_waf_cms_result.waf_detected
            if self.current_waf_cms_result.cms_detected:
                scan_results["CMS Detected"] = self.current_waf_cms_result.cms_detected
        ...
        messagebox.showinfo("Success",
                            f"Assessment report saved to:\n{filepath}\n\n"
                            "Open in browser and use Print > Save as PDF.")
        self._log(f"Assessment report exported: {filepath}", "success")
